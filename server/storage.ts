...
export interface IStorage {
  ...
  updateUser(userId: string, updates: { name?: string }): Promise<User | undefined>;
...
export class MemStorage implements IStorage {
  ...
  async updateUser(userId: string, updates: { name?: string }): Promise<User | undefined> {
  ...
  async createLiftOffer(insertOffer: InsertLiftOffer): Promise<LiftOffer> {
    ...
    const offer: LiftOffer = {
      ...insertOffer,
      id,
      fromW3W: insertOffer.fromW3W ?? null,
      toW3W: insertOffer.toW3W ?? null,
      earliestDepart: insertOffer.earliestDepart ?? null,
      latestArrive: insertOffer.latestArrive ?? null,
      availableSeats: insertOffer.availableSeats ?? 1,
      detourKm: insertOffer.detourKm ?? 10,
      priceType: insertOffer.priceType ?? "Free",
      vehicleType: insertOffer.vehicleType ?? "Hatchback",
      visibility: insertOffer.visibility ?? "Everyone",
      repeatPattern: insertOffer.repeatPattern ?? "None",
      autoGenerated: insertOffer.autoGenerated ?? false,
  ...
  async createLiftRequest(insertRequest: InsertLiftRequest): Promise<LiftRequest> {
    ...
    const request: LiftRequest = {
      ...insertRequest,
      id,
      fromW3W: insertRequest.fromW3W ?? null,
      toW3W: insertRequest.toW3W ?? null,
      earliestDepart: insertRequest.earliestDepart ?? null,
      latestArrive: insertRequest.latestArrive ?? null,
      hasBagsKit: insertRequest.hasBagsKit ?? false,
  ...
  async createBadge(badge: InsertBadge): Promise<Badge> {
    const badgeWithNulls: Badge = {
      ...badge,
      threshold: badge.threshold ?? null,
    };
    this.badges.set(badge.id, badgeWithNulls);
    return badgeWithNulls;
  }
  ...
  async createLoaderSpace(space: InsertLoaderSpace): Promise<LoaderSpace> {
    ...
    const newSpace: LoaderSpace = {
      ...
      priceType: space.priceType ?? "Quote",
      priceValue: space.priceValue ?? null,
      ...
      heightM: space.heightM ?? null,
      spaceAvailablePercent: space.spaceAvailablePercent ?? 100,
      acceptsCars: space.acceptsCars ?? true,
      acceptsVans: space.acceptsVans ?? false,
      acceptsBikes: space.acceptsBikes ?? false,
      acceptsRunning: space.acceptsRunning ?? true,
      acceptsNonRunners: space.acceptsNonRunners ?? false,
      acceptsSORN: space.acceptsSORN ?? false,
      strapsAvailable: space.strapsAvailable ?? true,
      winchAvailable: space.winchAvailable ?? false,
      rampsAvailable: space.rampsAvailable ?? false,
      enclosedTransport: space.enclosedTransport ?? false,
      cabSeatAvailable: space.cabSeatAvailable ?? false,
      ...
      departDate: space.departDate ?? null,
